...
import { ArrowLeft, Plus, X, ChevronLeft, ChevronRight, Package, Search } from "lucide-react";
...
export default function Stock() {
  ...
  const fetchBalances = useCallback(async () => {
    try {
      const { data, error } = await (supabase as any)
        ...
        .select("product_name, starting_balance")
        .order("product_name");
  ...
  const filteredProducts = stockSearch.length > 0
    ? balances.filter(b => b.product_name.toLowerCase().includes(stockSearch.toLowerCase()))
    : balances;

  const handleSelectProduct = (row: BalanceRow) => {
    setSelectedProduct(row);
    setStockSearch(row.product_name);
  ...
  const fg: React.CSSProperties = { color: "hsl(var(--foreground))" };
    ...
    <div className="min-h-screen" style={{ background: "hsl(var(--background))", color: "hsl(var(--foreground))" }}>
      ...
      <div className="flex justify-between items-center px-10 py-6 border-b" style={{ borderColor: border }}>
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <Package size={13} style={dim} />
            <span className="text-[11px] tracking-[0.2em] uppercase" style={dim}>Stock</span>
          </div>
          <span className="text-[13px] font-light">Boudoir</span>
        </div>
        <div className="flex items-center gap-6">
          <span className="text-[11px] tracking-wider" style={dim}>{todayDisplay}</span>
      ...
      <div className="max-w-[760px] mx-auto px-5 py-12">
        ...
        <div className="mb-12">
            ...
            {showDropdown && filteredProducts.length > 0 && (
              ...
              >
                {filteredProducts.map(row => (
                  ...
                  >
                    <span className="text-[13px] font-light">{row.product_name}</span>
                    <span className="text-[12px]" style={dim}>bal. {row.starting_balance}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Selected product balance display */}
          {selectedProduct && (
            <div className="surface-box p-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-[15px] font-light">{selectedProduct.product_name}</p>
              </div>
              <div className="text-right">
                <p
                  className="text-[32px] font-light leading-none"
                  style={{
                    color: selectedProduct.starting_balance <= 0
                      ? "hsl(var(--red))"
                      : selectedProduct.starting_balance <= 3
                        ? "hsl(var(--green))"
                        : "hsl(var(--foreground))"
                  }}
                >
                  {selectedProduct.starting_balance}
                </p>
            ...
            {entries.map((entry, idx) => (
              <div key={entry.id} className="flex items-center gap-2">
                ...
                >
                  ...
                  {balances.map(b => (
                    <option key={b.product_name} value={b.product_name}>{b.product_name}</option>
                  ))}
                </select>
                ...
                {log.map(row => (
                  <tr key={row.id} className="border-b table-row-hover" style={{ borderColor: border }}>
                    ...
                    <td className="text-[13px] font-light py-3 text-center">
                      <span style={{
                        color: row.ending_balance <= 0
                          ? "hsl(var(--red))"
                          : row.ending_balance <= 3
                            ? "hsl(var(--green))"
                            : "hsl(var(--foreground))"
                      }}>
                        {row.ending_balance}
