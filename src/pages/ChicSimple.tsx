...
const ChicSimple = ({ onBack, onBackToMain, products: propProducts }: ChicSimpleProps) => {
  ...
  const exportDepositCsv = (start: string, end: string) => {
    ...
    const rows = cashLog
      .filter(r => r.Branch === "Chic Nail Spa" && r.Date >= s && r.Date <= e2)
    ...
    a.href = url;
    a.download = "Chic Cash Export.csv";
    a.click();
  ...
  const refreshCashLog = async () => {
    ...
    const { data } = await (supabase as any)
      .from("Cash").select("*").eq("Branch", "Chic Nail Spa")
  ...
  const handleCashSubmit = async () => {
    ...
    try {
      for (const entry of toSubmit) {
        ...
        const payload = { Branch: "Chic Nail Spa", Date: entry.date, "Total GST": total, Credit: credit, QR: qr, Cash: cash, Error: errVal || null, Explanation: entry.errorNote || null };
        if (entry.existingId) {
      ...
      if (!cashError) {
        ...
        const { data: freshData } = await (supabase as any).from("Cash").select("*").eq("Branch", "Chic Nail Spa").gte("Date", fromDateStr);
      ...
      <div style={{
        ...
        >
          ...
          {confirmSuccess && lastConfirmedEntries && (() => {
            ...
            const confirmedGrn = `CHI ${dd}${mm}${yy}`;
            return (
