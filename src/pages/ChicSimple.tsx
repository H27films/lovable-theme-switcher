...
const ChicSimple = ({ onBack, onBackToMain, products: propProducts }: ChicSimpleProps) => {
  ...
  const toggleFavourite = async (product: OfficeProduct) => {
    const currentlyFav = isChicFav(product);
    ...
    await (supabase as any)
      ...
      .eq("id", product.id);
    setProducts(prev =>
      prev.map(p => p.id === product.id ? { ...p, "CHIC NAILSPA FAVOURITE": newVal } : p)
    );
    setSelectedProduct(prev =>
      prev && prev.id === product.id ? { ...prev, "CHIC NAILSPA FAVOURITE": newVal } : prev
    );
  };
  ...
  const orderFiltered = orderSearch.length > 0
    ...
    : products;
  const orderFavs    = orderFiltered.filter(p =>  isChicFav(p));
  const orderColours = orderFiltered.filter(p => !isChicFav(p) &&  isYes(p["Colour"]));
  const orderRegular = orderFiltered.filter(p => !isChicFav(p) && !isYes(p["Colour"]));
  ...
  const exportDepositCsv = (start: string, end: string) => {
    ...
    const rows = cashLog
      .filter(r => r.Branch === "Chic Nailspa" && r.Date >= s && r.Date <= e2)
    ...
    a.href = url;
    a.download = "Chic Nailspa Export.csv";
    a.click();
  ...
  const refreshCashLog = async () => {
    ...
    const fromDateStr = fromDate.toISOString().split("T")[0];
    const { data } = await (supabase as any)
      .from("Cash").select("*").eq("Branch", "Chic Nailspa")
  ...
  const handleCashSubmit = async () => {
    ...
    try {
      for (const entry of toSubmit) {
        ...
        const basePayload = { Branch: "Chic Nailspa", Date: entry.date, "Total GST": total, Credit: credit, QR: qr, Cash: cash, Error: errVal || null, Explanation: entry.errorNote || null };
        // Always check Supabase for existing row by Branch+Date
        const { data: existingRows } = await (supabase as any).from("Cash").select("id").eq("Branch", "Chic Nailspa").eq("Date", entry.date);
              ...
              <div style={{ flexShrink: 0, marginBottom: "24px", paddingBottom: "20px", borderBottom: "0.5px solid hsl(var(--border))" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", gap: "12px" }}>
                  ...
                  >
                    <Star
                      size={16}
                      fill={isChicFav(selectedProduct) ? "hsl(var(--foreground))" : "none"}
                      color="hsl(var(--foreground))"
                    />
                  </button>
                </div>
      ...
      <div style={{
        ...
        {showOrderDropdown && (
          <div style={{
            ...
            {(() => {
              ...
              const sections: React.ReactNode[] = [];
              if (orderFavs.length > 0)    { sections.push(sectionLabel("Chic Nailspa Favourites")); orderFavs.forEach(p => sections.push(renderRow(p, true))); }
        ...
        >
          ...
          {confirmSuccess && lastConfirmedEntries && (() => {
            ...
            const confirmedGrn = `CHIC ${dd}${mm}${yy}`;
            return (
