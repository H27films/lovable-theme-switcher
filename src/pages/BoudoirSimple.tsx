  ...
  const handleCashSubmit = async () => {
    ...
    try {
      for (const entry of toSubmit) {
        ...
        const basePayload = { Branch: "Boudoir", Date: entry.date, "Total GST": total, Credit: credit, QR: qr, Cash: cash, Error: errVal || null, Explanation: entry.errorNote || null };
        // Always check Supabase for existing row by Branch+Date
        const { data: existingRows } = await (supabase as any).from("Cash").select("id").eq("Branch", "Boudoir").eq("Date", entry.date);
      ...
      if (!cashError) {
        ...
        const { data: todayCheck } = await (supabase as any).from("Cash").select("id").eq("Branch", "Boudoir").eq("Date", todayStr);
