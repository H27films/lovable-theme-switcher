...
// easily recognisable in the log tables / pills. Medium-tuned so they
// work on the light, dark and sand themes alike.
const THERAPIST_PILL_PALETTE: { backgroundColor: string; color: string }[] = [
  ...
  { backgroundColor: "hsl(210 28% 44%)", color: "hsl(212 80% 96%)" }, // slate blue
  { backgroundColor: "hsl(155 28% 38%)", color: "hsl(150 50% 95%)" }, // sage green
  { backgroundColor: "hsl(275 30% 46%)", color: "hsl(275 70% 96%)" }, // dusty violet
];

// Deterministic pill style for a therapist. Returns real CSS style keys so it
// can be spread straight into a React `style` prop. The known THERAPISTS each
// get their own stable colour; any other name is hashed onto the palette.
